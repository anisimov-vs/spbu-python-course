...
from project.task3.carry import (
    curry_explicit,
    cache
)
...
def test_smart_args_isolated_deep_copy() -> None:
    @smart_args
    def modify_dict(*, d: Union[dict[str, str], Isolated] = Isolated()) -> dict[str, str]:
...
def test_smart_args_isolated_deep_copy_nested() -> None:
    @smart_args
    def g(*, d: Union[dict[str, dict[str, int]], Isolated] = Isolated()) -> dict[str, dict[str, int]]:
...
def test_smart_args_exact_task_example_with_random() -> None:
    ...
    @smart_args
    def check_evaluation(*, x: int = get_random_number(), y: Union[int, Evaluated] = Evaluated(get_random_number)) -> tuple[int, int]:
        return x, cast(int, y)
...
def test_smart_args_with_mixed_defaults() -> None:
    @smart_args
    def func(a: int, b: int = 10, *, c: Union[str, Evaluated] = Evaluated(lambda: "dynamic")) -> tuple[int, int, str]:
...
def test_smart_args_with_mixed_params_and_star_args() -> None:
    ...
    def inc() -> int:
        ...
        return counter
    @smart_args(positional_support=True)
    def f(a: Union[list[int], Isolated] = Isolated(), *args: Any, b: Union[int, Evaluated] = Evaluated(inc), **kwargs: Any) -> tuple[list[int], tuple[Any, ...], int, dict[str, Any]]:
...
def test_smart_args_positional_nested_isolation() -> None:
    @smart_args(positional_support=True)
    def f(data: Union[dict[str, dict[str, int]], Isolated] = Isolated()) -> dict[str, dict[str, int]]:
...
def test_smart_args_on_class_method() -> None:
    class MyClass:
        @smart_args
        def my_method(self, *, data: Union[dict[str, Any], Isolated] = Isolated()) -> dict[str, Any]:
...
def test_all_decorators_stacked() -> None:
    ...
    @cache(capacity=5)
    @smart_args(positional_support=True)
    def base_func(a: Union[tuple[int, ...], Isolated] = Isolated(), b: Union[tuple[int, ...], Isolated] = Isolated()) -> int:
